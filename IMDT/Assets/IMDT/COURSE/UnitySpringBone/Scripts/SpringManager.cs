        ...
        UpdateParameters();
            ...
            if (showAllColliders)
            {
                for (int i = 0; i < springBones.Length; i++)
                {
                    if (dynamicRatio > springBones[i].threshold)
                    {
                        int colliderCount = springBones[i].colliders.Length;

                        for (int c = 0; c < colliderCount; ++c) {
                            springBones[i].colliders[c].drawDebug = true;
                        }
            ...
            else
            {
                for (int i = 0; i < springBones.Length; i++)
                {
                    if (dynamicRatio > springBones[i].threshold)
                    {
                        ...
                        for (int c = 0; c < colliderCount; ++c)
                        {
                            springBones[i].colliders[c].drawDebug = false;
                        }
                    }
                }
            }
